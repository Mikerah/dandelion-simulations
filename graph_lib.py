...
class RegGraphGen(GraphGen):
    ...
    def generate_graph(self):
        ...
        for i in range(self.d - 1):
            ...
            # add a single hamiltonian cycle
...
class QuasiRegGraphGen(GraphGen):
    ...
    def assign_dandelion_nodes(self):
        ...
        dand_list = random.sample(
            [v for v in self.G.nodes() if not self.G.node[v]['spy']], n_honest)
    ...
    def generate_anon_graph(self):
        ...
        for n in self.G.nodes():
            ...
            out_edges = random.sample(
                candidates, min([self.d_anon, len(candidates)]))
        ...
        for node in self.G.nodes():
            ...
            if (self.k == 1):
                ...
                for connection in range(self.d):
                    ...
                    degrees = [
                        self.G.degree(i) if not self.G.node[i]['spy'] else 0 for i in choices]
                    # print 'degrees', degrees
                    min_degree_indices = [
                        i for i in range(
                            len(degrees)) if degrees[i] == min(degrees)]
